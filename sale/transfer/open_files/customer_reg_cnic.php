       ...
       let action = 'show';
            if ($("#CustomerRegistrationsCheckCnicForm").valid()) {
                ...
                success: function(response){
                    if(response.success)
                    {
                        $.get('sale/transfer/open_files/exist_customer_reg.php?cnic_no='+response.data[0].CNIC_NO,function(data,status){
                            $('#content').html(data);
                        });
                    }else{
                        ...
                        if (cnic_no.length == 13) {
                            $.get('sale/transfer/open_files/new_reg.php?cnic_no='+cnic_no,function(data,status){
                                $('#content').html(data);
                            });
                        }else{
