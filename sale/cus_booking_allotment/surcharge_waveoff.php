...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Surcharge waveoff</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <!-- <li class="breadcrumb-item"><a class="btn btn-sm"  href = "web-api/logout/logout.php">LOGOUT</a></li> -->
              <li class="breadcrumb-item active"><button class="btn btn-sm" id="surcharge_breadcrumb">Surcharge waveoff</button></li>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="modal-body">
                        <div class="modal-footer justify-content-between">
                            <button type="button" class="btn btn-default" data-dismiss="modal"><i class="fa fa-times"></i></button>
                            <button type="button" class="btn btn-info mt-2 btn-sm btn-Surcharge-challan" id="add_button">  
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                if(jQuery.inArray("402a5b_1", secF_dataArr) !== -1){
                    var inv_btn = '<button type="button" data-id='+form_no+' class="btn btn-sm btn-surcharge-invoice-view 402a5b_1" title="View Surcharge Invoice"><i class="fas fa-print text-info fa-fw" ></i></button>';
                }
...
$("#exampleModelSurcharge").on('click','.btn-Surcharge-challan',function(){
    ...
    if(ck_box < 1){
        ...
        $("#error_msg_Surcharge").text("remarks feils is required");
    }else{
        ...
        $.ajax({
            url: 'api/sales/cus_surcharge_api.php',
            type: 'POST',
            data: {action: 'surcharge_waveoff',totalAmount:totalAmount,checkamount:checkamount,booking_detail_id:checkValues,remarks:remarks,sale_id:sale_id,form_no:form_no},
            success: function(response){
                
                console.log(response);
                var status = response.status;
                var message = response.message;
                var booking_id = response.booking;
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        // let invoice_url = "invoicereports/customer_surcharge_challan.php?action=invoice_generate&FN="+form_no;
                        // window.open(invoice_url, '_blank');
                        // Payment reciept
                        let invoice_url = "invoicereports/wave_off_reciept.php?action=receipt_generate&bookingID="+booking_id+"&totalAmount="+totalAmount;
                        window.open(invoice_url, '_blank');
                    },
                    error: function(e) 
                    {
