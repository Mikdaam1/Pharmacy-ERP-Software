...
if(isset($_GET['action']) && $_GET['action'] == 'get_Checkboxes_all'){
    ...
    if(isset($_POST['tax_group_name']) && isset($_POST['tax_group_description']) && isset($_POST['check_value']) && (empty($_POST['tax_group_name']) || empty($_POST['tax_group_description']) || empty($_POST['check_value']))){
        ...
        $tax_group_name= ucfirst(preg_replace("/[^a-zA-Z .,_-]+/", "", $_POST['tax_group_name']));
        $tax_group_description = ucfirst(preg_replace("/[^a-zA-Z .,_-]+/", "", $_POST['tax_group_description']));
        $checked_values = $_POST['check_value'];
        $check_value = implode(',', $checked_values);

        $select="INSERT INTO COD_TAX_GROUP (TAX_GROUP,TAX_DESCRIPTION,COMPANY_ID,ACTION_TYPE,ACTION_ON,ACTION_BY,TAX_ID) VALUES ('$tax_group_name','$tax_group_description','$company_id','Insert','$current_date','$user_id','$check_value')";
        // print_r($select); die();
    ...
    while($row=oci_fetch_assoc($run)){
	    $return_data .= '<option data-code="'.$row['ID'].'" value="'.$row['ID'].'">'.$row['TAX_GROUP'].'</option>';
    }
    ...
    if(isset($_POST['id']) && !empty($_POST['id'])){
        ...
        $query = "SELECT * FROM COD_TAX_GROUP WHERE ID = '$id' AND DELETE_FLG = '0' AND COMPANY_ID = '$company_id'";
        $run = oci_parse($c, $query);
        if(oci_execute($run)){
    ...
    if(isset($_POST['id']) && empty($_POST['id'])){
        ...
        $select="UPDATE COD_TAX_GROUP SET TAX_GROUP = '$tax_group_name', TAX_DESCRIPTION = '$tax_group_description', TAX_ID = '$check_value', ACTION_TYPE ='Update', ACTION_ON ='$current_date', ACTION_BY ='$user_id' WHERE ID = '$id' AND COMPANY_ID = '$company_id'";
        //print_r($select); die();
        $run = oci_parse($c, $select);
