...
if(isset($_POST['action']) && $_POST['action'] == 'view'){
    ...
    $query = "SELECT A.VNO,A.VTYPE,A.VDATE,A.REMARKS,A.CHQDT,A.CHQNO,A.POSTED_BY,A.POSTED_ON,A.FISCAL_YEAR
    FROM V_MASTER A
    ...
    AND A.POSTED = 'Y' AND A.COMPANY_ID = '$company_id' AND A.PROJECT_ID = '$project_id' AND A.VTYPE = 'TR' 
    AND B.ACTIVE = 'Y' AND C.ACTIVE = 'Y' AND A.POSTED = 'Y' ORDER BY A.POSTED_ON DESC";
    // print_r($query);
    ...
    if(isset($_POST['id']) && !empty($_POST['id'])){
        ...
        WHERE B.ACTIVE = 'Y' AND C.ACTIVE = 'Y' AND A.VNO = '$id' AND A.VTYPE = 'TR' AND A.COMPANY_ID = '$company_id' 
        AND A.PROJECT_ID = '$project_id' ORDER BY A.REMARKS ASC";
        ...
        INNER JOIN COD_FISCAL_YEAR B ON B.SNO = A.FISCAL_YEAR
        INNER JOIN COD_DATE_LOCK C ON C.FISCAL_YEAR = A.FISCAL_YEAR 
        WHERE B.ACTIVE = 'Y' AND C.ACTIVE = 'Y' AND A.VNO = '$id' AND A.VTYPE = 'TR' AND A.COMPANY_ID = '$company_id' AND A.PROJECT_ID = '$project_id'";
...
}elseif(isset($_POST['action']) && $_POST['action'] == 'print_voucher'){
    print_r($_POST); die();
