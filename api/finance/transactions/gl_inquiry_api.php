...
if(isset($_POST['action']) && $_POST['action'] == 'gl_inquiry_statement'){
	if(isset($_POST['gl_acc_id']) && !empty($_POST['gl_acc_id'])){
		...
		// 					WHERE VDATE <= TO_DATE('".$pre_date."','YYYY-MM-DD') AND VDATE <= TO_DATE('".$ma."','YYYY-MM-DD') AND B.ID = '$id' GROUP BY B.ID";
		...
		//balance amount
		$balance_amount = "SELECT E.ID,SUM(A.DEBIT) AS DEBIT,SUM(A.CREDIT) AS CREDIT,SUM(A.DEBIT) - SUM(A.CREDIT) AS BALANCE
							FROM V_DETAIL A
							INNER JOIN V_MASTER B ON (B.VNO = A.VNO AND B.VTYPE = A.VTYPE AND B.FISCAL_YEAR = A.FISCAL_YEAR)
							INNER JOIN CHART_DETAIL E ON E.CHART_ACC_CODE = A.CHART_ACC_CODE
							INNER JOIN COD_FISCAL_YEAR C ON C.SNO = A.FISCAL_YEAR
							INNER JOIN COD_DATE_LOCK D ON D.FISCAL_YEAR = A.FISCAL_YEAR 
							WHERE B.VDATE >= TO_DATE('".$pre_date."','YYYY-MM-DD') AND B.VDATE <= TO_DATE('".$ma."','YYYY-MM-DD') AND E.ID = '$id'
							AND A.COMPANY_ID = '$company_id' AND A.PROJECT_ID = '$project_id' AND C.ACTIVE = 'Y' AND D.ACTIVE = 'Y' GROUP BY E.ID";
		// print_r($balance_amount);die();
		$compiled = oci_parse($c, $balance_amount);
		oci_execute($compiled);
		$row=oci_fetch_assoc($compiled);
		...
		// all data in table
		$all_data = "SELECT A.VNO,B.VDATE,A.VTYPE,A.DEBIT,A.CREDIT,A.FISCAL_YEAR
					FROM V_DETAIL A
					INNER JOIN V_MASTER B ON (B.VNO = A.VNO AND B.VTYPE = A.VTYPE AND B.FISCAL_YEAR = A.FISCAL_YEAR)
					INNER JOIN CHART_DETAIL E ON E.CHART_ACC_CODE = A.CHART_ACC_CODE
					INNER JOIN COD_FISCAL_YEAR C ON C.SNO = A.FISCAL_YEAR
					INNER JOIN COD_DATE_LOCK D ON D.FISCAL_YEAR = A.FISCAL_YEAR 
					WHERE B.VDATE >= TO_DATE('".$ma."','YYYY-MM-DD') AND B.VDATE <= TO_DATE('".$cd."','YYYY-MM-DD') AND E.ID = '$id'
					AND A.COMPANY_ID = '$company_id' AND A.PROJECT_ID = '$project_id' AND C.ACTIVE = 'Y' AND D.ACTIVE = 'Y' ORDER BY B.VDATE ASC";
