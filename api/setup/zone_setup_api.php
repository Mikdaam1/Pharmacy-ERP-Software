...
if(isset($_POST['action']) && $_POST['action'] == 'view'){
  // print_r("jfsksk");
    ...
    $query = "insert into zone(zone_code,zone_name) value ('$zone_code','$zone_name')";
    $result = $conn->query($query);
    // PRINT_R($query); die();
    if($result){
      $return_data = array(
        "status" => 1,
        "message" => "Zone has been Inserted"
    );
    }else{
        $return_data = array(
        "status" => 0,
        "message" => "Zone has not been Inserted"
        );
    }
}elseif(isset($_POST['action']) && $_POST['action'] == 'edit'){
  // print_r("jfsksk");
  $zone_code=$_POST['zone_code'];
    $query = "SELECT * FROM `zone` WHERE zone_code='$zone_code'";
    // PRINT_R($query);
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $show = mysqli_fetch_assoc($result);
    $return_data = $show;
}elseif(isset($_POST['action']) && $_POST['action'] == 'check_zone_code'){
    // print_r("jfsksk");
    $zoneCode=$_POST['inputzoneCode'];
    $query = "SELECT * FROM `zone` WHERE zone_code='$zoneCode'";
    // PRINT_R($query);
    // die();
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
  ...
  $description=$_POST['inputdescription'];
    $query = "SELECT zone_name FROM `zone` WHERE zone_name='$description'";
    // PRINT_R($query);
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $show = mysqli_fetch_assoc($result);
  ...
  $query="UPDATE `zone` set  zone_name = '$description' WHERE zone_code='$zone_code'";    
  $result = $conn->query($query);
  if($result){
      $return_data = array(
          "status" => 1,
      ...
      $return_data = array(
      "status" => 0,
      "message" => "Zone has not been updated"
      );
  }
