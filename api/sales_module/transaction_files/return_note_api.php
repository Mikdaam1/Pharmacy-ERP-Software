...
if(isset($_POST['action']) && $_POST['action'] == 'view'){
  // print_r("jfsksk");
    ...
    ('','$dc_key','$dc_key','$invoice_date','$company_code','$year','$doc_no','$doc_date','$purchase_mode','$division','$order_ref','$party_ref','$v_no','$party','$salesman','$ship_mode','$ship_no','$narration','$debit')";
    ...
    if($master_r){
        ...
        for($i=0;$i< count($_POST['acc_desc']); $i++){
            ...
            ('','$division','$company_code','$year','$doc_no','$doc_date','$acc_desc','$quantity','$narration','$purchase_mode','$loc','$dc_key','$dc_key','$invoice_date','$stax_amt','$stax_amt_e','$add_amt','$add_amt_e','$frt','$frt_e','$disc','$disc_e','$excl','$excl_e','$rate','$amount','$net_amt','','','$expiry_dt')"; 
            // print_r($detail_q);
  ...
  while($show = mysqli_fetch_assoc($result)){
    ...
    // print_r("jfsksk");
    $company_code=$_POST['company_code'];
    $query = "SELECT a.*,b.*,c.item_descr,d.party_name FROM stktran_dtl a inner join stktran_mst b on a.po_no=b.po_no and a.doc_no=b.doc_no and a.doc_year=b.doc_year left join items c on a.item_code=c.item_div
    left join party d on b.party_code=d.party_division
    WHERE b.co_code='$company_code'";
    // PRINT_R($query); DIE();  
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $return_data=[];
    ...
    AND a.po_no='$do_key' AND a.item_code='$item_code' AND a.doc_year='$year' AND a.qty='$quantity' AND a.rate='$rate'";
    // PRINT_R($query); DIE();  
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $return_data=[];
...
//   $dc_doc_no=$_POST['dc_doc_no'];
  $query = "SELECT a.receipt_qty,a.qty from dc_dtl a 
            WHERE a.item_code = '$item_code' AND a.po_no='$order_key' AND a.co_code='$company_code'";
  // PRINT_R($query); die();
  $select_r = $conn->query($query);
  $show = mysqli_fetch_assoc($select_r);
  $return_data=$show;
  ...
  $doc_date=$_POST['voucher_date'];
  $year=date('Y', strtotime($doc_date));
  $query = "SELECT DISTINCT a.*,b.po_no FROM items a inner join stktran_dtl b on a.item_div=b.item_code AND b.co_code='$company_code'
            AND b.div_code='$division_code' AND b.po_catg='$purchase_mode' AND b.doc_year='$year'";
  // PRINT_R($query); DIE();  
  $result = $conn->query($query);
  $count = mysqli_num_rows($result);
  $return_data=[];
