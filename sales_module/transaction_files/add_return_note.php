                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-4 form-group">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input pattern="[a-zA-Z0-9 ,._-]{1,}" maxlength="30" type="text" name="rt_ref" id="rt_ref" class="form-control form-control-sm" placeholder="Reference No" >
                                                </div>
                                            </div>
                                            <div class="col-sm-2 form-group">
                                                <label>Voucher#:</label> 
                                        ...
                                        <table id="example1" class="table table-bordered table-striped table-responsive">
                                            ...
                                            <tbody id="d_items">
                                                <tr id="main_tr">
                                                    ...
                                                    <td style="width: 30px;"><input  style="width:76px" type="text"  name="" id="dc_no" class="dc_no" readonly></td>
...
//select dc code
$('#order_form').on('click','.dc_no',function(){
    ...
    $('#dc_table').on('click','.even',function(){
        ...
        if(party_code !=''){
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'invoice_detail',company_code:company_code,party_code:party_code,doc_no:doc_no,do_key:do_key,item_code:item_code,doc_date:doc_date,quantity:quantity,rate:rate},
                dataType: "json",
                success: function(data){
                    ...
                    $('#add_amt'+id).val(data[0].addtax_rate);
                    $('#add_amt_e'+id).val(data[0].addtax_amt);
    ...
    $('#purchase_mode').change(function(){
      ...
      if(division_code != '' && company_code != '' && purchase_mode !=''){
        ...
        $.ajax({
            url: 'api/sales_module/transaction_files/return_note_api.php',
            type: 'POST',
            data: {action: 'item_code',company_code:company_code,division_code:division_code,purchase_mode:purchase_mode,voucher_date:voucher_date},
            dataType: "json",
            success: function(response){
                $("#ajax-loader").hide();
                // console.log(response);
...
$('#order_form').on('click','#order_key',function(){
        ...
        $.ajax({
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                table.row.add([response[i].doc_no,response[i].party_name,refnum,response[i].dates,response[i].po_no,response[i].doc,response[i].net_amt,response[i].od_qty,response[i].qty,'-',response[i].div_code,response[i].doc_year]);
...
$('#order_table').on('click','.even',function(){
    ...
    var order_doc=$(this).closest('tr').children('td:nth-child(6)').text();
    var total_net_amt=$(this).closest('tr').children('td:nth-child(7)').text();
    // console.log(total_net_amt);
    var qty=$(this).closest('tr').children('td:nth-child(8)').text();
    var receipt_qty=$(this).closest('tr').children('td:nth-child(9)').text();
    var order_bal=$(this).closest('tr').children('td:nth-child(10)').text();
    ...
    $.ajax({
        ...
        data: {action: 'get_detail',doc_no:doc_no,party_name:party_name,ref_num:ref_num,doc_date:doc_date,
          order_key:order_key,total_net_amt:total_net_amt,qty:qty,receipt_qty:receipt_qty,order_bal:order_bal,order_doc:order_doc},
        dataType: "json",
        async: false,
        success: function(response){
          console.log(response);
...
$('#order_table').on('click','.odd',function(){
    ...
    $.ajax({
        ...
        type: 'POST',
        data: {action: 'get_detail',doc_no:doc_no,party_name:party_name,ref_num:ref_num,doc_date:doc_date,
          order_key:order_key,qty:qty,receipt_qty:receipt_qty,order_bal:order_bal,order_doc:order_doc},
        dataType: "json",
        async: false,
        success: function(response){
        //   console.log(response);
    ...
    //   var dc_doc_no=$('#dc_doc_no').val();
      ...
      if(company_code != ''){
          ...
          $.ajax({
              url: 'api/sales_module/transaction_files/return_note_api.php',
              type: 'POST',
              data: {action: 'dc_detail',company_code:company_code},
              dataType: "json",
              success: function(response){
                  console.log(response);
              table.clear().draw();
              ...
              for (var i = 0; i < response.length; i++) {
                  sno++;
                  table.row.add([response[i].doc_no,response[i].item_descr,response[i].item_code,response[i].party_name,response[i].doc_date,response[i].po_no,response[i].qty,response[i].rate,response[i].amt,response[i].batch_no,response[i].item_type,response[i].g_d,response[i].gd_date,response[i].expiry_date,response[i].item_hscode,response[i].addtax_rate,response[i].party_code]);
